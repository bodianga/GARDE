...
class BranchRepository @Inject()(db: Database) {
  ...
  def findBranchByProjectId(branchId: Int, projectId: String): Seq[Branch] = {
    db.withConnection { implicit connection =>
      SQL" SELECT * FROM branch WHERE id = $branchId AND projectId = $projectId".as(parser.*)
    }
  }

  def existsByProjectId(branchId: Int, projectId: String): Boolean = {
    db.withConnection { implicit connection =>
      SQL"SELECT COUNT(*) FROM branch WHERE id = $branchId AND projectId = $projectId".as(scalar[Long].single) > 0
    }
  }
